...
def load_ground_thruth(path= "dane/ground_thruth.json"):
    if isfile(path):
...
def score_recognize(filename, text, truth, confusing):
        ...
        if text == text_ground_truth:
            ...
            for char1, char2 in zip(text, text_ground_truth):
                if char1 == char2:
                    ...
                    print(text, "should be", text_ground_truth, filename)
                    if text != 'PO12345':
                        confusing.append((char2, char1))
        return [score, points]
...
def achieved_pts(result, ground_thruth):
    ...
    for filename in result:
        score, total = score_recognize(
            filename, result[filename], ground_thruth, confusing)
        sum_of_scores += score
        sum_of_points += total
